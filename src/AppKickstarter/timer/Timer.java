...
public class Timer extends AppThread {
	...
	for (ActiveTimer timer : timeoutTimers) {
	    ...
	    mbox.send(new Msg("Timer", null, Msg.Type.TimesUp,null, "["+String.format("%05d", timerID)+"]: Time's up!"));
	...
	public void run() {
		...
		mbox.send(new Msg("Ticker", null, Msg.Type.Tick,null, "tick"));
	    }
	} // run
	...
	timerMBox.send(new Msg(id, mbox, Msg.Type.SetTimer,null,"set timer, "+sleepTime+", "+timerId));
	return timerId;
    } // setTimer
	...
	// chk msg sender
	if (!msg.getSender().equals("Timer") || msg.getType() != Msg.Type.TimesUp || !msg.getMessage().endsWith("]: Time's up!")) {
	    return -1;
	}
	String msgDetails = msg.getMessage();
	...
	String details = msg.getMessage().substring(11);
	...
	timerMBox.send(new Msg(id, mbox, Msg.Type.CancelTimer,null, "cancel timer, "+timerID));
    } // cancelTimer
